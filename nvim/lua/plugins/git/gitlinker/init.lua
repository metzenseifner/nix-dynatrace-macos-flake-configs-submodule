return {
  {
    ...
    config = function()
      ...
      local formatters = {
        {
          name = "Markdown Links",
          desc = "Format as [text](url) markdown links",
          format = function(url, text)
            -- Only escape square brackets in text, not parentheses
            text = text:gsub("[%[%]]", "\\%1")
        ...
        {
          name = "Plain URLs",
          desc = "Format as plain URLs with bullets",
          format = function(url, text)
            return "- " .. url
          end,
        },
      ...
      vim.keymap.set("n", "<leader>gYY", function()
        ...
        for i, formatter in ipairs(formatters) do
          table.insert(choices, string.format("%d. %s - %s", i, formatter.name, formatter.desc))
        end
                  ...
                  if filepath ~= "" then
                    pcall(function()
                      ...
                      if url then
                        ...
                        if text == "" then
                          text = filepath .. ":" .. item.lnum
                        end
                        table.insert(links, selected_formatter.format(url, text))
                      end
                    end)
                  end
                end
                ...
                vim.schedule(function()
                  ...
                  if #links > 0 then
                    ...
                    vim.schedule(function()
                      require("osc52").copy(result)
                      vim.notify(
                        string.format("✓ Copied %d formatted quickfix links to clipboard", #links),
                        vim.log.levels.INFO,
                        { timeout = 3000 }
                      )
                    end)
