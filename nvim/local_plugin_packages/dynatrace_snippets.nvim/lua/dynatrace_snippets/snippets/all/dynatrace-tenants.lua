...
snippet_generators.cloudcontrol_snippets = function(tenant)
  return s(string.format("cloudcontrol_%s (%s)", tenant.environment_id, tenant.saas_type ), {
    f(function(args, snip, user_arg_1)
      ...
      return string.format("[CloudControl for %s](https://cloudcontrol-%s.internal.dynatracelabs.com/#tenant;uuid=%s)", tenant.environment_id, tenant.saas_type, tenant.environment_id)
    end, {}),
  })
end
